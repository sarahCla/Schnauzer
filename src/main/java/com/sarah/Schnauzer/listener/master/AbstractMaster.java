...
public abstract class AbstractMaster implements IMaster {
	...
	private String[] getCommInsertSql(ColumnTypeHelper helper, WriteRowsEvent event, RepTableNew rep) throws UnsupportedEncodingException {
	...
	private String[] getInsUpSqlSql(ColumnTypeHelper helper, WriteRowsEvent event, RepTableNew rep) throws UnsupportedEncodingException {
		List<Row> rows = event.getRows();
		BitColumn bCol = event.getUsedColumns().clone();
	...
	private String[] getSql2000Sql(ColumnTypeHelper helper, WriteRowsEvent event, RepTableNew rep) throws UnsupportedEncodingException {
		List<Row> rows = event.getRows();
		BitColumn bCol = event.getUsedColumns().clone();
		String InsertSql = rep.getInsertFields(bCol);
		String[] sql = new String[rows.size()*4];
	...
	private String[] getInsertUpdateSql(ColumnTypeHelper helper, WriteRowsEvent event, RepTableNew rep) throws UnsupportedEncodingException {
		List<Row> rows = event.getRows();
		BitColumn bCol = event.getUsedColumns().clone();
		String InsertSql = rep.getInsertFields(bCol);
	...
	private String[] getInsertSql(ColumnTypeHelper helper, WriteRowsEvent event, ITableReplicator rep) throws UnsupportedEncodingException {
		if (rep.isMergeTable() && this.slaveDb.isMySQL()) {
			return getInsertUpdateSql(helper, event, (RepTableNew)rep);
		} else if (rep.isMergeTable() && this.slaveDb.isSQLServer()) {
			return getInsUpSqlSql(helper, event, (RepTableNew)rep);
		} else if (this.slaveDb.isSQLServer2000()) {
			return getSql2000Sql(helper, event, (RepTableNew)rep);			
		} else {
			return getCommInsertSql(helper, event, (RepTableNew)rep); 
		}
	}
	...
	public boolean doUpdate(ColumnTypeHelper helper, UpdateRowsEvent event) {
		...
		for (int i=0; i<tables.size(); i++)
		{
			RepTableNew rep =  (RepTableNew)tables.get(i);
