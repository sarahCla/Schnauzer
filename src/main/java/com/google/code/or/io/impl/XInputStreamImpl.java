...
public class XInputStreamImpl extends InputStream implements XInputStream {
	private static final Logger LOGGER = LoggerFactory.getLogger(XInputStreamImpl.class);
	//
	private int head = 0;
	private int tail = 0;
	...
	public int readInt(final int length) throws IOException {
		//LOGGER.info("Call readInt() begin ");
		int r = 0;
		for(int i = 0; i < length; ++i) {
			final int v = this.read();
			r |= (v << (i << 3));
		}
		//LOGGER.info("Call readInt() end ");
		return r;
	}
